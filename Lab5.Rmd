...
UScases <- time_series_deaths_long %>% 
  group_by(Date, Province_State) %>% 
  summarise(Deaths = sum(Deaths)) %>% 
  filter(Province_State %in% time_series_deaths_long$Province_State)

ggplot(data = UScases, mapping = aes(x = Date, y = Province_State, color = Province_State)) +
  geom_point() +
  geom_line() +
