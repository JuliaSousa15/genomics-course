...
report_09_26 <- read_csv(url("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_daily_reports/09-26-2020.csv")) %>%   rename(Long = "Long_") %>% 
  filter(Lat != "NA") %>% 
  filter(Long != "NA")
...
ggplotly(   
  ggplot(data = report_09_26, mapping = aes(x = long, y = lat, text = region, group = group)) +
    coord_fixed(1.3) +
...
updated_anisa <-   read_csv(url("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_daily_reports/09-26-2020.csv")) %>% 
  rename(Long = "Long_") %>% 
  filter(Country_Region == "US") %>% 
  filter (!Province_State %in% c("Alaska","Hawaii", "American Samoa",
                  "Puerto Rico","Northern Mariana Islands", 
...
mybreaks <- c(1, 100, 1000, 10000, 10000)
ggplot(updated_anisa, aes(x = Long, y = Lat, size = Confirmed)) +
    borders("state", colour = "white", fill = "grey90") +
    geom_point(aes(x=Long, y=Lat, size=Confirmed, color=Confirmed),stroke=F, alpha=0.7) +
...
[Link for static maps, interactive graphs, and references](lab6_references.html)
